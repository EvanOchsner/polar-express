...
df = pl.DataFrame({"id": ["doc1"], "data": [json.dumps(json_data)]})
...
high_rated_reviews = JSONPathExpr(
    "$.data.products[*].reviews[?(@.rating >= 4 && @.verified == true)].user", alias="high_rated_verified_reviewers"
)
...
mapper.add_select_step(
    [pl.col("id"), pl.col("product_names"), pl.col("high_rated_verified_reviewers"), pl.col("product_specs")]
)
